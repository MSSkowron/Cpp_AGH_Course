...
public:
    // constructors
    TwoDimensionMatrix();
    explicit TwoDimensionMatrix(const MatrixElement m[size][size]);
    TwoDimensionMatrix(const TwoDimensionMatrix &tdm) : TwoDimensionMatrix(tdm.matrix) {}
    // getters
    static constexpr size_t getSize() {return size;}
    [[nodiscard]] MatrixElement get(int row,int column) const;
    // overloading operators
    TwoDimensionMatrix& operator=(TwoDimensionMatrix const & tdm);
    TwoDimensionMatrix& operator*=(MatrixElement number);
    TwoDimensionMatrix operator&&(const TwoDimensionMatrix& m) const;
    MatrixElement * operator[](size_t i);
    const MatrixElement * operator[](size_t i) const;
    operator size_t() const;
    //friends
    friend std::ostream & operator<<(std::ostream & os, const TwoDimensionMatrix & t);
    friend std::istream & operator>>(std::istream &is, TwoDimensionMatrix &t);
